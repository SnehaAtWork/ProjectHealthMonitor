...
# You MUST update this map based on your new analysis from analyze_clusters_v2.py
# The meaning of the cluster numbers (0, 1, 2, 3 etc.) will have changed.
CLUSTER_MAP = {
    0: "Healthy / On Track", 
    1: "At Risk / Schedule Concerns", 
    2: "In Trouble / Budget & Blocker Issues", 
    3: "Needs Attention / Scope Creep" 
    # This is an EXAMPLE. Update with your actual findings.
}
# This MUST match the feature list from your training script
# --- THIS IS THE ONLY CHANGE MADE TO THIS SCRIPT ---
...
def predict_health_status():
    ...
    # Create a full DataFrame from the incoming data
    full_df = pd.DataFrame(features_data)

    # Extract the project IDs to reattach them later
    project_ids = full_df['projectID'].tolist()

    # Create the numeric-only DataFrame for the model, ensuring column order
    ...
    # Assemble the response by combining the stored project_ids with the new labels
    results = [
